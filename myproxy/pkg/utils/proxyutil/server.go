...
func NewServer(filebase string, apiProxyPrefix string, staticPrefix string, filter *FilterServer, cfg *rest.Config, keepalive time.Duration, appendLocationPath bool) (*Server, error) {
	...
	//apiProxyPrefix需要在config的server加后缀，多集群的另外一种做法是把ctxName作为apiProxyPrefix，然后注册不同的Handler
	mux.Handle(apiProxyPrefix, WrapperProxyHandler(proxyHandler)) //tips 这里包装了自己的handler
	if filebase != "" {
...
// tips 自建server，把apiconfig里面所有的handler都handle起来
func NewServerUponMultiCluster(filebase string, apiProxyPrefix string, staticPrefix string, filter *FilterServer, keepalive time.Duration, appendLocationPath bool) (*Server, error) {
	mux := http.NewServeMux()
	restMap := NewClusterService().GenerateRestMap()
	handlers := map[string]http.Handler{}
	defaultCtx := ""
	for ctxName, restConf := range restMap {
		proxyHandler, err := NewProxyHandler(apiProxyPrefix, filter, restConf.restConfig, keepalive, appendLocationPath)
		//apiProxyPrefix := fmt.Sprintf("/%s/", ctxName)
		if err != nil {
			return nil, err
		}
		if restConf.isDefault {
			defaultCtx = ctxName
		}
		handlers[ctxName] = proxyHandler
	}
	mux.Handle(apiProxyPrefix, &MyProxyHandler{handlers: handlers, defaultCtx: defaultCtx})
	if filebase != "" {
		// Require user to explicitly request this behavior rather than
		// serving their working directory by default.
		mux.Handle(staticPrefix, newFileHandler(staticPrefix, filebase))
...
func NewProxyHandler(apiProxyPrefix string, filter *FilterServer, cfg *rest.Config, keepalive time.Duration, appendLocationPath bool) (http.Handler, error) {
	...
	//tips 做了包装了自己的transport
	proxy := proxy.NewUpgradeAwareHandler(target, WrapperTransport(transport), false, false, responder)
