...
type podWorkers struct {
	...
	// PodManager is used to set cache
	podManager kubepod.Manager //自己加的 源码没有
}
...
func NewPodWorkers(
	...
	podCache kubecontainer.Cache,
	pf *PodFn,
	podManager kubepod.Manager, //额外加的，为了自动set cache
) PodWorkers {
	return &podWorkers{
		podSyncStatuses:                    map[types.UID]*podSyncStatus{},
		podUpdates:                         map[types.UID]chan podWork{},
		...
		startedStaticPodsByFullname:        map[string]types.UID{},
		waitingToStartStaticPodsByFullname: map[string][]types.UID{},
		syncPodFn:                          pf.SyncPodFn,
		syncTerminatingPodFn:               pf.SyncTerminatingPodFn,
		syncTerminatedPodFn:                pf.SyncTerminatedPodFn,
		recorder:                           recorder,
		workQueue:                          queue.NewBasicWorkQueue(cl),
		resyncInterval:                     time.Second * 1,
...
func insertPodCache(pod *v1.Pod, podCache kubecontainer.Cache) {
	// 其实应该从podManager里获取到pod实例更合理
	podStatus := GetPodStatusReady(pod)
	podCache.Set(pod.UID, podStatus, nil, time.Now())
}
...
func (p *podWorkers) managePodLoop(podUpdates <-chan podWork) {
	...
	for update := range podUpdates {
		...
		//fmt.Println("要处理的pod名称是"+pod.Name, ", pod的uid是"+string(pod.UID))

		// 自动为新的pod设置为ready并塞入cache
		//insertPodCache(pod, p.podCache)
